...
export async function getCostKpis(w: Window, hosts: string[] | null = null): Promise<CostKpis> {
  ...
  const pDayStart = ds(prior.start)
  const pDayEnd = ds(prior.end)
  ...
  const inTok = Number(cacheRow?.in_tok ?? 0)
  const cacheTok = Number(cacheRow?.cache ?? 0)
  const inTokPrior = Number(cacheRow?.in_tok_prior ?? 0)
  const cachePrior = Number(cacheRow?.cache_prior ?? 0)
  ...
  const topModel = modelRows[0]?.model
    ? {
        ...
        pctOfCost: totalModelCost > 0 ? Number(modelRows[0].cost) / totalModelCost : 0,
      }
    : null
  ...
  const priorTopModel = topModel
    ? modelRowsPrior.find((r) => r.model === topModel.model)
    : undefined
  const topModelPctPrior =
  ...
  return {
    ...
    activeSessions: {
      count: active.length,
      sample: active.map((a) => ({ sessionId: a.session_id, projectPath: a.project_path })),
    },
  }
}
