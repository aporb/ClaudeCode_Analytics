...
describe('computeBriefing', () => {
  it('omits delta line when prior period has zero spend', () => {
    const b = computeBriefing({
      windowCost: 10,
      windowCostPriorPeriod: 0,
      cacheHitPct: 0.5,
      cacheHitPctPrior: 0.4,
  ...
  it('uses today so far label when partial day', () => {
    const b = computeBriefing({
      windowCost: 5,
      windowCostPriorPeriod: 4,
      cacheHitPct: 0.5,
      cacheHitPctPrior: 0.5,
  ...
  it('emits dash for delta when prior data missing', () => {
    const b = computeBriefing({
      windowCost: 12,
      windowCostPriorPeriod: Number.NaN,
      cacheHitPct: 0.5,
      cacheHitPctPrior: 0.5,
  ...
  it('renders to plain string', () => {
    const b = computeBriefing({
      windowCost: 100,
      windowCostPriorPeriod: 80,
      cacheHitPct: 0.31,
      cacheHitPctPrior: 0.52,
