...
describe('backfill: host opt threading', () => {
  ...
  beforeAll(async () => {
    ...
    cpSync(join(PARSER_FIXTURES, 'file-history'), join(tempHome, 'file-history'), { recursive: true })
    cpSync(join(PARSER_FIXTURES, 'shell-snapshots'), join(tempHome, 'shell-snapshots'), { recursive: true })
  ...
  it('stamps host on every row across all 8 tables when host=hostinger', async () => {
    ...
    const populated = ['events','sessions','messages','prompts_history','todos','file_snapshots','shell_snapshots'] as const
    for (const table of populated) {
      const rows = await sql.unsafe(`SELECT host FROM ${table}`)
      expect(rows.length, `${table} must have rows for the assertion to be meaningful`).toBeGreaterThan(0)
      expect(rows.every((r) => r.host === 'hostinger'), `every ${table}.host must be 'hostinger'`).toBe(true)
    }
    ...
    expect(toolCalls.every((r) => r.host === 'hostinger'), `every tool_calls.host must be 'hostinger'`).toBe(true)
  })
})
